...
KERNEL_REPEATS_MIN=200
KERNEL_REPEATS_STEP=200
...
for ((i=$KERNEL_REPEATS_MIN; i<=$KERNEL_REPEATS_MAX; i+=$KERNEL_REPEATS_STEP))
do
...
if [ "$MACHINE" = "spa" ] ; then
    sh send-encrypted.sh -k qGE5Pn -p Archimedes -s klvlqmhb -t "spa: tuning test complete" -m "spa: tuning test completed! Time taken: $DIFF seconds"
fi

if [ "$MACHINE" = "XXXII" ] ; then
    sh send-encrypted.sh -k qGE5Pn -p Archimedes -s klvlqmhb -t "XXXII: tuning test complete" -m "XXXII: tuning test completed! Time taken: $DIFF seconds"
fi
