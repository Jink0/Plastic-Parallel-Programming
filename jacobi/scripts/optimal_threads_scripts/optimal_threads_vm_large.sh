...
if [ "$MACHINE" = "spa" ] ; then
    ...
    STRING="0..11 "
    UPDATE_MOTD=false
    FILENAME="configs/spa/optimal_threads_vm_large.ini"
    LOG_FILENAME="logs/spa/optimal_threads_vm_large.log"
fi
...
if [ "$MACHINE" = "XXXII" ] ; then
    ...
    STRING="0..31 "
    UPDATE_MOTD=true
    FILENAME="configs/XXXII/optimal_threads_vm_large.ini"
    LOG_FILENAME="logs/XXXII/optimal_threads_vm_large.log"
fi
...
if [ "$MACHINE" = "spa" ] ; then
    sh send-encrypted.sh -k qGE5Pn -p Archimedes -s klvlqmhb -t "spa: experiment complete" -m "spa: optimal threads vm large completed! Time taken: $DIFF seconds"
fi

if [ "$MACHINE" = "XXXII" ] ; then
    sh send-encrypted.sh -k qGE5Pn -p Archimedes -s klvlqmhb -t "XXXII: experiment complete" -m "XXXII: optimal threads vm large completed! Time taken: $DIFF seconds"
fi
