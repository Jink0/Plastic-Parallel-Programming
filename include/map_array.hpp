...
void map_array(struct workload<in1, in2, out>& work, deque<out>& output) {
    ...
    rgstr.pid                      = pid;
    rgstr.settings.schedule        = work.params.schedule;
    ...
    while (bot.empty == false)
    {
      ...
      if (msg.header != -1)
      {
        ...
        if (work.params.thread_pinnings.size() > new_num_threads)
        {
          ...
          join_with_threads(threads, work.params.thread_pinnings.size() - new_num_threads);
    ...
    // Terminating threads.
    /*bot.thread_control.assign(work.params.thread_pinnings.size(), Terminate);

    // Joining with threads.
    join_with_threads(threads, work.params.thread_pinnings.size());

    // Update parameters.
    work.params.schedule = msg.settings.schedule;
    ...
    bot.thread_control.assign(work.params.thread_pinnings.size(), Execute);
    ...
    // Reset thread ids.
    threads.clear();
    threads.resize(work.params.thread_pinnings.size());

    // Create all our needed threads.
    for (uint32_t i = 0; i < work.params.thread_pinnings.size(); i++)
    {
      print("[Main] Creating thread ", i , "\n");
